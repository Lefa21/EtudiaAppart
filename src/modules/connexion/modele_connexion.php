...
class ModeleConnexion extends Connexion
{
    ...
    public function ajoutUtilisateur()
    {
        ...
        if (
            ...
            $sql = Connexion::getBdd()->prepare(
                "SELECT * FROM joueur WHERE username = :login"
            );
            ...
            if ($sql->rowCount() >= 1) {
                ...
                $hashMotDePasse = password_hash(
                    $_POST["password"],
                    PASSWORD_DEFAULT
                );
                $password = $hashMotDePasse;
                $sql = Connexion::getBdd()->prepare(
                    "INSERT INTO joueur (username,password,money,img_profil,online) VALUES (:login,:password,0,null,true)"
                );
    ...
    public function connexionUtilisateur()
    {
        if (
            ...
            if (
                isset($_SESSION["identifiant_utilisateur"]) &&
                $_SESSION["identifiant_utilisateur"] == $_POST["login"]
            ) {
                echo 'Vous êtes déjà connecté sous l\'identifiant : ' .
                    $_SESSION["identifiant_utilisateur"] .
                    "<br>";
                return;
            } else {
                $login = $_POST["login"];
    ...
    public function deconnexionUtilisateur()
    {
        if (isset($_SESSION["identifiant_utilisateur"])) {
            echo "Déconnexion réussie, identifiant :" .
                $_SESSION["identifiant_utilisateur"] .
                "<br>";
